...
exports.createCategory = (req, res) => {
  if (!req.body.category_name)
    return res
      .status(400)
      .json({ message: "Please provide a valid data for new Category" });
    ...
    connection.query(
      ...
      (error, result) => {
        ...
        if (error) {
          ...
          res.status(201).json({
            message: `Category ${req.body.category_name} is successfully added.`,
            category_id: lastInsertedId_category
          });
        }
      }
    );
...
exports.updateOneCategory = (req, res) => {
  if ((!req.body.category_id, !req.body.category_name))
    return res
      .status(400)
      .json({ message: "Please provide a valid info about Category" });
  ...
  pool.getConnection((error, connection) => {
    ...
    connection.query(
      "UPDATE `CATEGORY` SET `CATEGORY_NAME` = ? WHERE `CATEGORY`.`CATEGORY_ID` = ?",
      ...
      (error, result) => {
        ...
        if (error) {
          ...
          if (result.affectedRows == 0) {
            ...
            console.log(result);
            res.status(201).json({
              message: `Category with id=${req.body.category_id} is successfully updatet to ${req.body.category_name}.`
            });
          }
        }
      }
...
exports.deleteOneCategory = (req, res) => {
  if (!req.body.category_id)
    return res
      .status(400)
      .json({ message: "Please provide a valid categroy ID" });
      ...
      (error, result) => {
        ...
        if (error) {
          ...
          if (result.affectedRows == 0) {
            ...
            res.status(200).json({
              message: `Category with id: ${req.body.category_id} is successfully deleted.`
            });
          }
        }
      }
