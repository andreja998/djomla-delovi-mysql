...
exports.getParts = (req, res) => {
  ...
  pool.getConnection((error, connection) => {
    ...
    switch (option) {
      case 1:
        sqlQuery =
          "SELECT P.PART_ID, P.`PART_NAME`, P.`PART_PRICE`, P.`PART_DESC`, PI.PICTURE_DEST, PI.PICTURE_NAME, M.MODEL_NAME, MK.MAKER_NAME, C.CATEGORY_NAME, SC.SUBCATEGORY_NAME FROM `PART_CATEGORY_SUBCATEGORY` AS PCS LEFT JOIN `PART` AS P ON PCS.PART_ID = P.PART_ID JOIN `MODEL` AS M ON PCS.MODEL_ID = M.MODEL_ID AND M.MODEL_ID = ? JOIN `MAKER` AS MK ON M.MAKER_ID = MK.MAKER_ID AND MK.MAKER_ID = ? LEFT JOIN `CATEGORY_SUBCATEGORY` AS CS ON PCS.CATEGORY_SUBCATEGORY_ID = CS.CATEGORY_SUBCATEGORY_ID LEFT JOIN `CATEGORY` AS C ON CS.CATEGORY_ID = C.CATEGORY_ID LEFT JOIN `SUBCATEGORY` AS SC ON CS.SUBCATEGORY_ID = SC.SUBCATEGORY_ID LEFT JOIN (SELECT PIC.PICTURE_DEST, PIC.PICTURE_NAME, PIC.PART_ID FROM `PICTURE` AS PIC GROUP BY PIC.PART_ID) AS PI ON P.PART_ID = PI.PART_ID";
        break;
      case 2:
        sqlQuery =
      ...
      case 3:
        sqlQuery =
          "SELECT P.PART_ID, P.`PART_NAME`, P.`PART_PRICE`, P.`PART_DESC`, PI.PICTURE_DEST, PI.PICTURE_NAME, M.MODEL_NAME, MK.MAKER_NAME, C.CATEGORY_NAME, SC.SUBCATEGORY_NAME FROM `PART_CATEGORY_SUBCATEGORY` AS PCS LEFT JOIN `PART` AS P ON PCS.PART_ID = P.PART_ID JOIN `MODEL` AS M ON PCS.MODEL_ID = M.MODEL_ID AND M.MODEL_ID = ? JOIN `MAKER` AS MK ON M.MAKER_ID = MK.MAKER_ID AND MK.MAKER_ID = ? LEFT JOIN `CATEGORY_SUBCATEGORY` AS CS ON PCS.CATEGORY_SUBCATEGORY_ID = CS.CATEGORY_SUBCATEGORY_ID AND CS.CATEGORY_ID = ? AND CS.SUBCATEGORY_ID = ? JOIN `CATEGORY` AS C ON CS.CATEGORY_ID = C.CATEGORY_ID AND C.CATEGORY_ID = ? LEFT JOIN `SUBCATEGORY` AS SC ON CS.SUBCATEGORY_ID = SC.SUBCATEGORY_ID LEFT JOIN (SELECT PIC.PICTURE_DEST, PIC.PICTURE_NAME, PIC.PART_ID FROM `PICTURE` AS PIC GROUP BY PIC.PART_ID) AS PI ON P.PART_ID = PI.PART_ID";
      case 4:
        sqlQuery =
            "SELECT P.PART_ID, P.`PART_NAME`, P.`PART_PRICE`, P.`PART_DESC`, PI.PICTURE_DEST, PI.PICTURE_NAME, M.MODEL_NAME, MK.MAKER_NAME, C.CATEGORY_NAME, SC.SUBCATEGORY_NAME FROM `PART_CATEGORY_SUBCATEGORY` AS PCS LEFT JOIN `PART` AS P ON PCS.PART_ID = P.PART_ID JOIN `MODEL` AS M ON PCS.MODEL_ID = M.MODEL_ID `MAKER` AS MK ON M.MAKER_ID = MK.MAKER_ID LEFT JOIN `CATEGORY_SUBCATEGORY` AS CS ON PCS.CATEGORY_SUBCATEGORY_ID = CS.CATEGORY_SUBCATEGORY_ID LEFT JOIN `CATEGORY` AS C ON CS.CATEGORY_ID = C.CATEGORY_ID LEFT JOIN `SUBCATEGORY` AS SC ON CS.SUBCATEGORY_ID = SC.SUBCATEGORY_ID LEFT JOIN (SELECT PIC.PICTURE_DEST, PIC.PICTURE_NAME, PIC.PART_ID FROM `PICTURE` AS PIC GROUP BY PIC.PART_ID) AS PI ON P.PART_ID = PI.PART_ID";
        break;
    }
